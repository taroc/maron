...
ipc.on('load-markdown', function(event, filepath, isViewIndex){
    ...
    watcher.on('change', function(){
        fs.readFile(filepath[0], 'utf8', function (err, mdtext) {
            if (err) throw error;
            window.render_markdown(mdtext, filedir, isViewIndex);
        });
    });
...
ipc.on('reload-markdown', function(event, filepath, isViewIndex){
