...
ipc.on('open-markdown', function(event, filepath){
    ...
    webview.send('load-markdown', filepath, isViewIndex);
});
...
ipc.on('reload', function(event){
    if(markdownPath != ''){
        console.log('reload')
        webview.send('reload-markdown', markdownPath, isViewIndex);
    }else{
        webview.reload();
    }
});

ipc.on('toggle-index', function(event){
